...
if __name__ == '__main__':
    ...
    parser.add_argument('--pred-only', dest='pred_only',
                        action='store_true', help='only display the prediction')
    ...
    if os.path.isfile(args.img_path):
        ...
        filenames = [os.path.join(args.img_path, filename)
    ...
    for filename in tqdm(filenames):
        ...
        depth = (depth - depth.min()) / (depth.max() -
                                         depth.min()) * np.iinfo(np.uint16).max
        ...
        if args.pred_only:
            cv2.imwrite(os.path.join(
                args.outdir, filename[:filename.rfind('.')] + '_depth.png'), depth)
        else:
            split_region = np.ones(
                (raw_image.shape[0], margin_width, 3), dtype=np.uint8) * 255
            combined_results = cv2.hconcat([raw_image, split_region, depth])

            caption_space = np.ones(
                (caption_height, combined_results.shape[1], 3), dtype=np.uint8) * 255
            ...
            for i, caption in enumerate(captions):
                ...
                text_size = cv2.getTextSize(
                    caption, font, font_scale, font_thickness)[0]
            ...
            cv2.imwrite(os.path.join(
                args.outdir, filename[:filename.rfind('.')] + '_img_depth.png'), final_result)
